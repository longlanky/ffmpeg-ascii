 ...
 * @param {number} [args.offset] - seconds already played (seek position; display only)
 * @param {function} [args.clock] - ms time source (default Date.now; inject a fake in tests)
 */
function createScheduler({ fps, duration = null, offset = 0, clock = Date.now } = {}) {
  ...
  return {
    ...
    /** Media position in seconds (seek offset + time since first frame). */
    elapsed() {
      return base + now() / 1000;
    },

    /** One-line status for stderr: `00:12/01:30 · eff 19fps · out 142 (dropped 8)`. */
    status() {
      const elapsedSec = base + now() / 1000;
