...
- `lib/ascii.js`: `asciiFromBGRA()` single-pass BGRA→ASCII LUT, intensity `(r+g+b)` matching legacy `asciify-pixel` ramp `" .,:;i1tfLCG08@"`; truecolor FG `\x1b[38;2;…m` per char when `--color` (default), `stripAnsi()` for `--output` files.
