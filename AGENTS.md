...
- `lib/ascii.js`: `asciiFromBGRA()` single-pass BGRA→ASCII LUT, intensity `(r+g+b)` matching legacy `asciify-pixel` ramp `" .,:;i1tfLCG08@"`; truecolor FG `\x1b[38;2;…m` per same-color run + one trailing reset when `--color` (default), opt-in Bayer `--dither` (±½ ramp step, clamped at extremes); `stripAnsi()` for `--output` files.
- `lib/diff.js`: pure cell-grid diff (`diffGrids` → cursor-addressed `runsToAnsi`); `main.js` repaints only changed cells on display (`--no-diff` disables, >60% changed falls back to full draw, `--output` files always full frames); scheduler presentation clock anchors on first decoded frame (spawn latency must not cause opening drops).
...
- `main.js`: commander wiring only — `play` streams with cursor hide/show + clear, `space`/`q` raw-mode controls, `SIGWINCH` respawn (restarts from frame 0), `--loop` via `-stream_loop -1`; exports `playStream/renderOnce/resolveRenderOptions` for reuse. Guarded `require.main === module` so tests can require without parsing argv.
